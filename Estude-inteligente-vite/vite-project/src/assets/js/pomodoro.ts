...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const studyTimeInput = document.getElementById('study-time') as HTMLInputElement; // Input para tempo de estudo
    const shortBreakTimeInput = document.getElementById('short-break-time') as HTMLInputElement; // Input para tempo de pausa curta
    const longBreakTimeInput = document.getElementById('long-break-time') as HTMLInputElement; // Input para tempo de pausa longa
    const longBreakIntervalInput = document.getElementById('long-break-interval') as HTMLInputElement; // Input para intervalo de pausa longa
    const totalSessionsInput = document.getElementById('total-sessions') as HTMLInputElement; // Input para total de sessões no ciclo
    ...
    const inputTarefa = document.getElementById('tarefa') as HTMLInputElement; // Input para adicionar nova tarefa
    const botaoAdicionar = document.getElementById('adicionar') as HTMLInputElement; // Botão para adicionar tarefa
    ...
    function updateDisplay() {
        ...
        if (sessionTypeDisplay) sessionTypeDisplay.textContent = getPhaseName(currentPhase); // Atualiza o tipo de sessão
    ...
    function endCycle() {
        ...
        if (timerDisplayArea) timerDisplayArea.innerHTML = `<span class="text-3xl md:text-4xl font-bold">Sessão Encerrada</span>`;
        if (progressIndicatorArea) progressIndicatorArea.classList.add('hidden'); // Oculta o indicador de progresso
        if (sessionInfoArea) sessionInfoArea.classList.add('hidden'); // Oculta as informações da sessão
    ...
    function restoreUI() {
        ...
        if (sessionInfoArea) sessionInfoArea.classList.remove('hidden'); // Exibe as informações da sessão

        // Restaura os botões de controle do timer
        if (buttonsArea) buttonsArea.innerHTML = `
        ...
        if (newResetTimer) newResetTimer.addEventListener('click', resetTimer);
        if (newHandleTimerEnd) newHandleTimerEnd.addEventListener('click', handleTimerEnd);
    }
    ...
    function criarElementoTarefa(tarefa: {texto: string, concluida: boolean}) {
        ...
        botaoRemover.addEventListener('click', () => {
            if (listaTarefas) listaTarefas.removeChild(novaTarefa); // Remove a tarefa
    ...
    if (newResetBtn) newResetBtn.addEventListener('click', resetTimer);
    const newCompleteRoundBtn = document.getElementById('complete-round-btn');
    if (newCompleteRoundBtn) newCompleteRoundBtn.addEventListener('click', handleTimerEnd);

    // Salvar configurações
    if (saveTimeSettingsBtn) saveTimeSettingsBtn.addEventListener('click', () => {
