...
document.addEventListener('DOMContentLoaded', (): void => {
    ...
    if (!quizForm) {
        console.warn('Formulário de quiz não encontrado na página');
        return;
    }

    // Interface para tipagem de respostas
    interface LearningStyles {
        [key: string]: number;
    }

    // Adiciona um event listener para o evento de 'submit' do formulário
    quizForm.addEventListener('submit', function (event: SubmitEvent): void {
        event.preventDefault(); // Previne o comportamento padrão de recarregar a página ao submeter o formulário

        const form = event.target as HTMLFormElement; // O formulário que disparou o evento
        const answers: LearningStyles = {}; // Objeto para armazenar a contagem de cada tipo de resposta
        ...
        radios.forEach((radio: Element) => {
            ...
            const type = inputElement.value; // Obtém o valor do rádio (ex: 'visual', 'auditivo')
        ...
        let mostFrequentType: string | null = null; // Variável para armazenar o tipo de aprendizagem mais frequente
        ...
        if (mostFrequentType && descriptions[mostFrequentType]) {
            resultText = descriptions[mostFrequentType];
        } else if (mostFrequentType) {
            resultText = 'Não foi possível determinar um estilo predominante. Tente responder novamente!';
        } else {
        ...
        if (resultElement) {
            ...
            console.error('Elemento de resultado não encontrado na página');
        }
    });
});